...
2. **Crawl & Render Layer**: Invokes `audit_crawl()`. If the target is unreachable (HTTP $\ge 400$ or network failure, `F-CRAWL-001`), the orchestrator triggers **Causal Error Shielding**, immediately halting downstream skill execution to prevent alert pollution from error pages.
...
- **Causal Error Shielding (`run_audit.py`)**: If the origin site is unreachable or returns an HTTP error status (`F-CRAWL-001`), downstream content and schema checks are shielded rather than emitting cascading false positives on HTTP error pages.
...
| **RFC 9309 Path-Scoped Disallows** | Treats any non-empty `Disallow:` (e.g. `/webstats/`, `/admin/`) as a site-wide block, triggering false critical alarms on real websites. | Enforces RFC 9309 root semantics: narrow path-scoped disallows leave the site indexable, while root `Disallow: /` without an `Allow: /` override flags a critical block. | `test_20_path_scoped_robots_disallow_guard` |
